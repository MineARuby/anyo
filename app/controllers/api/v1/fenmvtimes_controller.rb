class Api::V1::FenmvtimesController < Api::V1::BaseController
  def index
    @start_time = Time.now.to_f
    @file_name = "tmp/d" + SecureRandom.hex(10) + ".txt"
  ...
  def deal_with_script
    pid = fork { exec("bin/fenmvtime_script.exp #{@mvtime[:value]} '" + @fen[:value].to_s + "' > #{@file_name}") }
    ...
    render json: { fen: @param_fen, pv: variation_ct, cp_score: cpscore, benchmark: bmark }, status: 200
  end
end
